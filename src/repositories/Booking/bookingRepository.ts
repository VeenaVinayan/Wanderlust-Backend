  ...
  implements IBookingRepository
{
    ...
    try {
      ...
      const [data, totalCount] = await Promise.all([
        this._bookingModel
          ...
          .populate({
            path: "packageId",
            select: "name images price description day night itinerary agent",
          })
          .skip((page - 1) * perPage)
          .limit(perPage)
          .exec(),
  ...
  async getBookingCompleteData(
    bookingId: string
  ): Promise<IBookingCompleteData> {
    const data = await this._bookingModel.aggregate([
  ...
  async validateBooking(
    packageId: string,
    tripDate: Date
  ): Promise<IBookingValidationResult> {
    const startDate = new Date(tripDate);
    startDate.setDate(startDate.getDate() - 5);

    const endDate = new Date(tripDate);
    endDate.setDate(endDate.getDate() + 30);
    const packageValue = await this._packageModel
      .findOne({ _id: packageId }, { totalCapacity: 1 })
      .lean();
