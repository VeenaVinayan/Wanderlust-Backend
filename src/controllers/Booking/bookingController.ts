...
export class BookingController {
  ...
  getAgentBookingData = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const { id } = req.params;
        const filterParams: FilterParams = {
        ...
        const bookings = await this._bookingService.getAgentBookingData(
          filterParams
        );
        if (bookings) {
          res.status(HttpStatusCode.OK).json({ success: true, bookings });
        } else {
          res
            .status(HttpStatusCode.NOT_FOUND)
  ...
  updateBookingStatusByAgent = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        if (!bookingId || !status) {
          res
            ...
            .json({
              success: false,
              message: StatusMessage.MISSING_REQUIRED_FIELD,
            });
          return;
        }
        const response = await this._bookingService.updateBookingStatusByAgent(
          bookingId,
          status
        );
        if (response) {
  ...
  getBookingDataToAdmin = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const filterParams: FilterParams = {
          page: Number(req.query.page),
        ...
        const bookings = await this._bookingService.getBookingDataToAdmin(
          filterParams
        );
        if (bookings) {
          res.status(HttpStatusCode.OK).json({ success: true, bookings });
  ...
  cancelBooking = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const result: CancellBookingResult =
          await this._bookingService.cancelBooking(String(bookingId));
        switch (result) {
          ...
          case CancellBookingResult.EXCEEDED_CANCELLATION_LIMIT:
            res
              ...
              .json({
                success: false,
                message: StatusMessage.EXCEEDED_CANCELLATION_LIMIT,
              });
            return;
          case CancellBookingResult.ALREADY_CANCELLED:
            res
              ...
              .json({
                success: false,
                message: StatusMessage.ALREADY_CANCELLED,
              });
            return;
          default:
            res
              .status(HttpStatusCode.NOT_FOUND)
              .json({ success: false, message: StatusMessage.NOT_FOUND });
        }
      } catch (err) {
        next(err);
      }
  ...
  getPackageBooking = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const data = await this._bookingService.getPackageBookingData(
          filterParams
        );
  ...
  validateBooking = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const values: IBookingValidationResult | null =
          await this._bookingService.validateBooking(
            String(packageId),
            new Date(String(day))
          );
        res.status(HttpStatusCode.OK).json({ success: true, values });
      } catch (err) {
        next(err);
      }
    }
