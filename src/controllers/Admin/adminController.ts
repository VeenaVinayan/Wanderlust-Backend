...
export class AdminController {
  ...
  getAllData = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const users = await this._adminService.getAllData(
          user,
          parseInt(perPage),
          parseInt(page),
          search,
        ...
        res
          ...
          .json({ success: true, message: StatusMessage.SUCCESS, users });
      } catch (err) {
        next(err);
      }
    }
  ...
  blockOrUnblock = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const { userId } = req.body;
        const response = await this._adminService.blockOrUnblock(userId);
        if (response) {
          res
            .status(HttpStatusCode.OK)
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        if (response) {
          res
            .status(HttpStatusCode.OK)
            .json({ message: StatusMessage.CREATED });
        } else {
          res
  ...
  getCategories = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const result: TCategoryResult = await this._adminService.getCategories(
          filterParams
        );
        const categories: TCategoryDTO[] = categoryMapper.categoryMapper(
          result.categories
        );
        const data = {
          categories,
      ...
      try {
        ...
        if (!categoryId) {
          res
            ...
            .json({ meessage: StatusMessage.MISSING_REQUIRED_FIELD });
        }
  ...
  editCategory = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const response = await this._adminService.editCategory(
          categoryId,
          category
        );
        if (response) {
  ...
  pendingAgentData = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const agentData = await this._adminService.getPendingAgentData(
          filterParams
        );
  ...
  rejectAgentRequest = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const { agentId } = req.params;
