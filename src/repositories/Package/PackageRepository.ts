  ...
  implements IPackageRepository
{
  ...
  async advanceSearch(queryString: QueryString): Promise<TPackageResult> {
    try {
      ...
      const matchStage: FilterQuery<IPackage> = {};
      const sortStage: FilterQuery<IPackage> = {};
      ...
      if (searchQuery.price) {
        if (searchQuery.price === "50000+") {
          ...
          const [minPrice, maxPrice] = (searchQuery.price as string)
            .split("-")
            .map(Number);
