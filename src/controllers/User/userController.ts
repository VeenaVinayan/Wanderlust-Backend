...
export class UserController {
      ...
      try {
        ...
        if (data) {
          ...
          res
            ...
            .json({ message: StatusMessage.ERROR });
        }
      } catch (err) {
        next(err);
      }
        ...
        switch (response) {
          case 1:
            res
              ...
              .json({
                error: true,
                message: StatusMessage.OLD_PASSWORD_INCORRECT,
              });
            return;
          case 2:
            res
              ...
              .json({
                success: true,
                message: StatusMessage.PASSWORD_RESET_SUCCESS,
              });
            return;
          case 3:
            res
              .status(HttpStatusCode.BAD_REQUEST)
              .json({ error: true, message: StatusMessage.PASSWORD_MISMATCH });
            return;
          case 4:
            res
              .status(HttpStatusCode.NOT_FOUND)
              .json({ error: true, message: StatusMessage.USER_NOT_FOUND });
            return;
          default:
            res
              .status(HttpStatusCode.OK)
  ...
  getPackages = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const packages = await this._userService.getPackages();
        if (packages) {
          res
            .status(HttpStatusCode.OK)
            .json({ message: StatusMessage.SUCCESS, packages });
        } else {
          res
            .status(HttpStatusCode.NO_CONTENT)
            .json({ message: StatusMessage.ERROR });
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        if (data) {
          ...
          res
            .status(HttpStatusCode.NO_CONTENT)
            .json({ message: StatusMessage.ERROR, data });
        }
      } catch (err) {
        next(err);
      }
  ...
  getReview = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const review = await this._userService.getReview(
          String(userId),
          String(packageId)
        );
        if (review) {
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        if (!packageId) {
          res
            .status(HttpStatusCode.BAD_REQUEST)
            .json({ error: true, message: StatusMessage.BAD_REQUEST });
          return;
        }
  ...
  editReview = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const result = await this._userService.editReview(
          { review, rating },
          reviewId
        );
        if (result) {
          res
            .status(HttpStatusCode.OK)
            .json({ success: true, message: StatusMessage.UPDATE_SUCCESS });
        } else {
          res
            .status(HttpStatusCode.BAD_REQUEST)
            .json({ success: false, message: StatusMessage.UPDATE_FAILED });
        }
      } catch (err) {
        next(err);
      }
  ...
  getUserDetails = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const { userId } = req.params;
        const data: IAgentChatDataDTO | null =
          await this._userService.userDetails(userId);
        res
          .status(HttpStatusCode.OK)
          .json({ message: StatusMessage.SUCCESS, data });
      } catch (err) {
