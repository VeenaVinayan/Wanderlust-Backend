...
export class PackageController {
  ...
  editPackage = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const { packageId } = req.params;
        const packageData = req.body;
        const result = await this._packageService.editPackage(
          packageId,
          packageData
        );
        if (result) {
  ...
  getPackages = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const result: TPackageResult = await this._packageService.findPackages(
          filterParams
        );
        const packages: TPackageDataDTO[] = packageMapper.userPackageData(
          result.packages
        );
        const data = {
          packages: packages,
  ...
  getAgentPackages = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const packages: TPackageResult =
          await this._packageService.findAgentPackages(filterParams);
        res
          .status(HttpStatusCode.OK)
  ...
  getCategoryPackages = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      try {
  ...
  advanceSearch = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      try {
          const result: TPackageResult = await this._packageService.advanceSearch(
          req.query
        );
        const packages: TPackageDataDTO[] = packageMapper.userPackageData(
          result.packages
  ...
  verifyPackage = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const { packageId } = req.params;
        const { value } = req.body;
        const response = await this._packageService.verifyPackage(
          packageId,
          value
        );
        if (response) {
